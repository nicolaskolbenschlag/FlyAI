...
def split_video(video_file, image_name_prefix):    
    return subprocess.check_output('ffmpeg -i "' + os.path.abspath(video_file) + '" '+ os.path.join(destination_path, 'JPEGImages', image_name_prefix +'%d.jpg'), shell=True)#, cwd=os.path.join(destination_path, 'JPEGImages'))
...
def calculate_share(num_training_images, num_val_images, num_testing_images):
    ...
    for scene in videos_to_be_processed:
        ...
        path = os.path.join(dataset_path, 'video', scene)
...
if __name__ == '__main__':
    ...
    dataset_path = 'data/sdd_kaggle'

    destination_folder_name = "sdd_voc"
    destination_path = os.path.join("data", destination_folder_name)
